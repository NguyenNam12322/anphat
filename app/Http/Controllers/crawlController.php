...
http://dienmaynguoiviet.com/dieu-hoa-Funiki-SH09MMC-2-chieu-9000BTU/";
    ...
    public function crawlAnPhat()
    {
        $urls  = 'https://www.anphatpc.com.vn/laptop-acer-swift-3-sf314-71-74wd-nx.kawsv.001.html';

        $html =  $this->getDom(trim($urls));

        $content = $html->find(".js-static-container",0);

        $specialDetail = $html->find('.item-content', 0);
    ...
    public function getLinkAnPhat()
    {

        for ($i=1; $i < 75; $i++) { 
            
            $link ='https://www.anphatpc.com.vn/thiet-bi-van-phong.html?page='.$i;

            $dom = $this->getDom($link);

            foreach ($dom->find('.product-list-container .p-text .p-name') as $links) {
                
                DB::table('crawl_link')->insert(['link'=>$links->href, 'group_id'=>1, 'group_link'=>$link]);
            }
        }
